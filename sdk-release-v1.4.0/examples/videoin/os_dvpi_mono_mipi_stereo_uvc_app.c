...
#include "rotate.h"
#include "mjpeg.h"
#include "inc/os_dvpi_mono_mipi_stereo_uvc_app.h"
#include "rsz.h"
...
static char* TAG = "os_dvpi_mono_mipi_stereo_uvc_app";

static pthread_t m_handle = NULL;

static int m_thread_created = 0;
static videoin_context_t *m_videoin_context_dvpi = NULL;
static videoin_context_t *m_videoin_context_mipi = NULL;
...
static os_dvpi_mono_mipi_stereo_param_t m_param = {
    .videoin_id_dvpi   = VIDEOIN_ID_DVPI0,
    .sens_type_dvpi    = "ov7725dvpi",
    .videoin_id_l = VIDEOIN_ID_MIPI2,
    .videoin_id_r = VIDEOIN_ID_MIPI1,
    .sens_type_mipi    = "ov02b",    
    .fps          = 30,
};
...
static void *os_dvpi_mipi_uvc_app_thread_entry(void *param)
{
    ...
    while (m_thread_created)
    {
        ...
        if (toggle == 1) {
            context = m_videoin_context_mipi;
        } else {
            context = m_videoin_context_dvpi;
        } 
...
int os_dvpi_mono_init(void)
{
    ...
    videoin_id_t id = m_param.videoin_id_dvpi;
    const char *sens_type = m_param.sens_type_dvpi;
    ...
    m_videoin_context_dvpi = videoin_init(&config);
    if (m_videoin_context_dvpi == NULL) {
        LOGE(__func__, "videoin_init dvpi fail");
		return -1;
    }

    ret = videoin_start_stream(m_videoin_context_dvpi);
    if (ret < 0) {
        videoin_release(m_videoin_context_dvpi);
        m_videoin_context_dvpi = NULL;
...
int os_mipi_stereo_init(void)
{
    ...
    const char *sens_type = m_param.sens_type_mipi;
    ...
    m_videoin_context_mipi = mipi_stereo_camera_init(id_l, id_r, drv[id_l], drv[id_r]);
    if (m_videoin_context_mipi == NULL) {
...
fail_out:
    os_dvpi_mono_mipi_stereo_uvc_app_exit();
    return -1;
}
...
int os_dvpi_mono_mipi_stereo_uvc_app_exit(void)
{
    ...
    if (m_thread_created) {
        ...
        if (m_videoin_context_mipi != NULL) {
            mipi_stereo_camera_uninit(m_videoin_context_mipi);
			m_videoin_context_mipi = NULL;
        }        
...
int main(int argc, char *argv[])
{
    ...
    os_app_set_custom_entry(os_dvpi_mono_mipi_stereo_uvc_app_entry);
