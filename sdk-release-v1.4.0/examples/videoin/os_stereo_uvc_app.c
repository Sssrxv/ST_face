...
#include "rotate.h"
#include "mjpeg.h"
#include "inc/os_stereo_uvc_app.h"
#include "rsz.h"
#include "os_isp_core.h"
#include "os_startup.h"

static char* TAG = "os_stereo_uvc_app";
...
static os_stereo_uvc_param_t m_param = {
...
static void *os_stereo_uvc_app_thread_entry(void *param)
{
    LOGI(__func__, "Start...");
...
int os_stereo_uvc_app_entry(void)
{
    ...
    const char *uvc_dev_name = "uvc_bulk_gadget";
    ...
    m_videoin_context = mipi_stereo_camera_init(id_l, id_r, drv[id_l], drv[id_r]);
    ...
    port_set_pthread_name("stereo_uvc_app");
    pthread_attr_init(&attr);
    pthread_attr_setstacksize(&attr, 16*1024);


    ret = pthread_create(&m_handle, &attr, os_stereo_uvc_app_thread_entry, NULL);
    if (ret < 0) {
        goto fail_out;
    }
...
fail_out:
    os_stereo_uvc_app_exit();
    return -1;
}

int os_stereo_uvc_app_exit(void)
{
    LOGI(__func__, "exit os_stereo_uvc_app() ...");
    ...
    if (m_thread_created) {
        ...
        if (m_videoin_context != NULL) {
            mipi_stereo_camera_uninit(m_videoin_context);
...
{
    LOGI(TAG, "AIVA MIPI STEREO UVC EXAMPLE");

    /* Register custom entry */
    os_app_set_custom_entry(os_stereo_uvc_app_entry);
