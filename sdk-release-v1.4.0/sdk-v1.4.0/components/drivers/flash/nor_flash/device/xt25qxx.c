...
#define xt25qxx_FLASH_PAGE_SIZE              256
#define xt25qxx_FLASH_SECTOR_SIZE            4096
#define xt25qxx_FLASH_PAGE_NUM_PER_SECTOR    16
...
static int _xt25qxx_quad_page_program(uint32_t addr, uint8_t *data_buf, uint32_t length);
static int _xt25qxx_page_program(uint32_t addr, uint8_t *data_buf, uint32_t length);
static int xt25qxx_quad_read_data(uint32_t addr, uint8_t *data_buf, uint32_t length);
static int xt25qxx_read_data(uint32_t addr, uint8_t *data_buf, uint32_t length);

static page_program_func_t m_page_program_func;
static read_func_t m_read_func;
static int xt25qxx_wait_for_ready();


static int xt25qxx_receive_data(uint8_t *cmd_buff, uint8_t cmd_len, uint8_t *rx_buff, uint32_t rx_len)
{
    int ret;
...
static int xt25qxx_send_data(uint8_t *cmd_buff, uint8_t cmd_len, uint8_t *tx_buff, uint32_t tx_len)
{
    int ret;
    spi_init(m_spi_bus_no, SPI_WORK_MODE_0, SPI_FF_STANDARD, FIFO_W8);
#ifndef FLASH_USE_DMA
...
static int xt25qxx_receive_data_enhanced(uint32_t *cmd_buff, uint8_t cmd_len, uint8_t *rx_buff, uint32_t rx_len)
{
    int ret;
...
static int xt25qxx_send_data_enhanced(uint32_t *cmd_buff, uint8_t cmd_len, uint8_t *tx_buff, uint32_t tx_len)
{
    int ret;
...
static int xt25qxx_write_enable(void)
{
    int ret;
    uint8_t cmd[1] = {WRITE_ENABLE};

    ret = xt25qxx_send_data(cmd, 1, 0, 0);
    return ret;
}


static int xt25qxx_write_status_reg1(uint8_t reg1_data)
{
    int ret;
    uint8_t cmd[2] = {WRITE_REG1, reg1_data};

    ret = xt25qxx_write_enable();
    if (ret < 0) {
        return ret;
    }
    ret = xt25qxx_send_data(cmd, 2, 0, 0);
    if (ret < 0) {
        return ret;
    }
    ret = xt25qxx_wait_for_ready();
    return ret;
}


static int xt25qxx_write_status_reg2(uint8_t reg2_data)
{
    int ret;
...
static int xt25qxx_read_status_reg1(uint8_t *reg_data)
{
    int ret;
    uint8_t cmd[1] = {READ_REG1};
    uint8_t data[1] = {0};

    ret = xt25qxx_receive_data(cmd, 1, data, 1);
    if (ret < 0) {
        return ret;
    }
    *reg_data = data[0];
...
static int xt25qxx_read_status_reg2(uint8_t *reg_data)
{
    int ret;
...
static int xt25qxx_enable_quad_mode(void)
{
    int ret;
    uint8_t reg2_data = 0;

    ret = xt25qxx_read_status_reg2(&reg2_data);
    if (ret < 0) {
        return ret;
    }
    if (!(reg2_data & REG2_QUAL_MASK)) {
        reg2_data |= REG2_QUAL_MASK;
        xt25qxx_write_status_reg2(reg2_data);
    }
    ...
    m_read_func = xt25qxx_quad_read_data;
    m_page_program_func = _xt25qxx_quad_page_program;

    return FLASH_OK;
}


static int xt25qxx_write_prot_cfg(uint8_t prot_value)
{
    int ret;
    uint8_t reg_data1 = 0;

    ret = xt25qxx_read_status_reg1(&reg_data1);
    if (ret < 0) {
        return ret;
    }
    ...
    xt25qxx_write_status_reg1(reg_data1);

    return FLASH_OK;
}


static int xt25qxx_disable_quad_mode(void)
{
    int ret;
    uint8_t reg2_data = 0;
    ret = xt25qxx_read_status_reg2(&reg2_data);
    ...
    if (reg2_data & REG2_QUAL_MASK) {
        reg2_data &= (~REG2_QUAL_MASK);
        ret = xt25qxx_write_status_reg2(reg2_data);
    }
    ...
    m_read_func = xt25qxx_read_data;
    m_page_program_func = _xt25qxx_page_program;
    return ret;
}


static int xt25qxx_is_busy(void)
{
    uint8_t status = 0;

    xt25qxx_read_status_reg1(&status);
...
int xt25qxx_sector_erase(uint32_t addr)
{
    ...
    ret = xt25qxx_send_data(cmd, 4, 0, 0);
    if (ret < 0) {
        return ret;
    }
    if (xt25qxx_wait_for_ready() < 0) {
        return -FLASH_ERROR_TIMEOUT;
    }
    return FLASH_OK;
}
...
static int _xt25qxx_quad_page_program(uint32_t addr, uint8_t *data_buf, uint32_t length)
{
    ...
    ret = xt25qxx_send_data_enhanced(cmd, 2, data_buf, length);
    if (ret < 0) {
        return ret;
    }
    if (xt25qxx_wait_for_ready() < 0) {
...
static int _xt25qxx_page_program(uint32_t addr, uint8_t *data_buf, uint32_t length)
{
    ...
    ret = xt25qxx_send_data(cmd, 4, data_buf, length);
    if (ret < 0) {
        return ret;
    }
...
int xt25qxx_page_program(uint32_t addr, uint8_t *data_buf, uint32_t length)
{
    ...
    int ret;
    ret = m_page_program_func(addr, data_buf, xt25qxx_FLASH_PAGE_SIZE);
    return ret;
}


static int xt25qxx_sector_program(uint32_t addr, uint8_t *data_buf)
{
    uint8_t index = 0;
    int ret;

    for (index = 0; index < xt25qxx_FLASH_PAGE_NUM_PER_SECTOR; index++)
    {
        ret = xt25qxx_page_program(addr, data_buf, xt25qxx_FLASH_PAGE_SIZE);
        if (ret < 0) {
            return ret;
        }
        addr += xt25qxx_FLASH_PAGE_SIZE;
        data_buf += xt25qxx_FLASH_PAGE_SIZE;
    }
    return FLASH_OK;
}
...
static int xt25qxx_quad_read_data(uint32_t addr, uint8_t *data_buf, uint32_t length)
{
    ...
    ret = xt25qxx_receive_data_enhanced(cmd, 2, data_buf, length);
...
static int xt25qxx_read_data(uint32_t addr, uint8_t *data_buf, uint32_t length)
{
    ...
    ret = xt25qxx_receive_data(cmd, 4, data_buf, length);
...
int xt25qxx_read_id(uint8_t *manuf_id, uint16_t *device_id)
{
    ...
    ret = xt25qxx_receive_data(cmd, 1, data, 3);
    if (ret < 0) {
        return ret;
    }
    if (data[0] != 0x0B) {
...
int xt25qxx_read_unique_id(flash_unique_id_t *unique_id)
{
    ...
    ret = xt25qxx_receive_data(cmd, 1, data, 20);
    if (ret < 0) {
        return ret;
    }
...
int xt25qxx_write(uint32_t addr, uint8_t *data_buf, uint32_t length)
{
    ...
    uint8_t *pread = NULL;
    uint8_t *pwrite = NULL;
    /*uint8_t swap_buf[xt25qxx_FLASH_SECTOR_SIZE] = {0};*/
    uint8_t *swap_buf = aiva_malloc(xt25qxx_FLASH_SECTOR_SIZE);
    ...
    while (length)
    {
        sector_addr   = addr & (~(xt25qxx_FLASH_SECTOR_SIZE - 1));
        sector_offset = addr & (xt25qxx_FLASH_SECTOR_SIZE - 1);
        sector_remain = xt25qxx_FLASH_SECTOR_SIZE - sector_offset;
        write_len     = ((length < sector_remain) ? length : sector_remain);
        ret = xt25qxx_read(sector_addr, swap_buf, xt25qxx_FLASH_SECTOR_SIZE);
        if (ret < 0) {
            goto out;
        }
        ...
        for (index = 0; index < write_len; index++) {
            if ((*pwrite) != ((*pwrite) & (*pread))) {
                ret = xt25qxx_sector_erase(sector_addr);
                if (ret < 0) {
                    goto out;
                }
                break;
        ...
        if (write_len == xt25qxx_FLASH_SECTOR_SIZE) {
            ret = xt25qxx_sector_program(sector_addr, data_buf);
        }
        else {
            pread = swap_buf + sector_offset;
            pwrite = data_buf;
            for (index = 0; index < write_len; index++)
                *pread++ = *pwrite++;
            ret = xt25qxx_sector_program(sector_addr, swap_buf);
        }
        if (ret < 0) {
            goto out;
        }
...
int xt25qxx_init(uint8_t spi_index, uint8_t spi_ss, uint32_t rate, int quad_rw)
{
    ...
    ret = xt25qxx_read_id(&manuf_id, &device_id);
    if (ret < 0) {
        return ret;
    }
    ...
    switch (device_id) {
        case 0x6018: // p25q128
            ...
            m_param.dev_name = "XT25Q128D";
            break;
        case 0x4018:
            m_chip_size = 16 * 1024 * 1024; // 16MB
            m_param.dev_name = "XT25F128F-W";
            break;
        default:
            return -FLASH_UNSPPORT;
    }
    ...
    m_param.sector_size = xt25qxx_FLASH_SECTOR_SIZE;
    m_param.page_size   = xt25qxx_FLASH_PAGE_SIZE;

    if (quad_rw) {
        ret = xt25qxx_enable_quad_mode();
    }
    else {
        ret = xt25qxx_disable_quad_mode();
    }
...
int xt25qxx_lock(uint32_t addr, uint32_t length)
{
    ...
    xt25qxx_write_prot_cfg(feature_prot_cfg_value);
    return FLASH_OK;
}
...
int xt25qxx_unlock(uint32_t addr, uint32_t length)
{
    ...
    xt25qxx_write_prot_cfg(0);
    return FLASH_OK;
}


#define XTX_NOR_OPERATION \
        .init         = xt25qxx_init,\
        .read         = xt25qxx_read,\
        .write        = xt25qxx_write,\
        .page_prog    = xt25qxx_page_program,\
        .sector_erase = xt25qxx_sector_erase,\
        .get_param    = xt25qxx_get_param,\
        .lock         = xt25qxx_lock,\
        .unlock       = xt25qxx_unlock,\
        .read_unique_id = xt25qxx_read_unique_id

static nor_flash_drv_t xt25qxx_nor_drivers[] = {
    // xtx NOR
    {
        .manuf_id     = 0x0B,
        .device_id    = 0x6018,
 ...
 */
UDEVICE_EXPORT(UCLASS_SPI_FLASH,     xt25qxx_nor_0b_6018,    0,      &xt25qxx_nor_drivers[0],    NULL);
UDEVICE_EXPORT(UCLASS_SPI_FLASH,     xt25qxx_nor_0b_4018,    0,      &xt25qxx_nor_drivers[1],    NULL);
