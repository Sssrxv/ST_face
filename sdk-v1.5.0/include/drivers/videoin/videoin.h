...
typedef struct videoin_config {
    ...
    void               *frame_mem_pools0[3];
    void               *frame_mem_pools1[3];// two camera separate mode will use.
} videoin_config_t;
...
typedef struct videoin_context
{
    ...
    frame_buf_t     *frame_bufs[VIDEOIN_ID_MAX];
} videoin_context_t;
...
uint32_t videoin_get_frame_index(videoin_id_t id);
uint32_t videoin_get_frame_time_stamp(videoin_id_t id);
