...
static int i80_jlt28024a_get_params(i80_lcd_driver_t *dev_driver, display_i80_params_t *param)
{
    (void)dev_driver;
    param->width   = g_width;
...
#if 0
static int i80_jlt28024a_scan_dir(int scan_dir)
{
    uint32_t cmd;
...
static int i80_jlt28024a_display_dir(int dir)
{
    if (dir == 0) {
        //vertical display
        i80_jlt28024a_scan_dir(U2D_R2L);
    } else {
        //horizontal display
        i80_jlt28024a_scan_dir(L2R_U2D);
    }
    return 0;
}

static int i80_jlt28024a_init(void)
{
    uint32_t cmd;
    uint16_t cmd_param[16];
    ...
    LOGI(__func__, "DISPLAY ID: 0x%x, 0x%x, 0x%x, 0x%x\n", (int)cmd_result[0], (int)cmd_result[1], (int)cmd_result[2], (int)cmd_result[2]);
    ...
    cmd_param[0]    = 0x1D; //VRH[5:0]
    i80_ctl_send_wr_cmd(cmd, &cmd_param[0], 1);

    cmd             = 0xC1; //Power control
    cmd_param[0]    = 0x12; //SAP[2:0]; BT[3:0]
    i80_ctl_send_wr_cmd(cmd, &cmd_param[0], 1);
    ...
    i80_jlt28024a_display_dir(0);
    return 0;
}
#else
static int i80_jlt28024a_init(i80_lcd_driver_t *dev_driver)
{
    uint32_t cmd;
    uint16_t cmd_param[16];
    uint16_t cmd_result[16];
...
static int i80_jlt28024a_suspend(i80_lcd_driver_t *dev_driver)
{
...
static int i80_jlt28024a_resume(i80_lcd_driver_t *dev_driver)
{
    dev_driver->set_backlight(dev_driver, 1);
...
static void i80_jlt28024a_set_backlight(i80_lcd_driver_t *dev_driver, uint32_t value)
{
...
static i80_lcd_driver_t i80_jlt28024a = {
    .name                   = "i80_jlt28024a",
    .context                = NULL,
    .backlight_pin          = GPIO_PIN2,

    .init                   = i80_jlt28024a_init,
    .suspend                = i80_jlt28024a_suspend,
    .resume                 = i80_jlt28024a_resume,
    .set_backlight          = i80_jlt28024a_set_backlight,
    .set_reset_pin          = NULL,
    .get_interface_param    = i80_jlt28024a_get_params,
};

/* Export Device:
 * 	    [uclass ID], [device name], [flags], [driver], [private pointer]
 */
UDEVICE_EXPORT(UCLASS_DISPLAY_I80,     i80_jlt28024a,    0,      &i80_jlt28024a,    NULL);