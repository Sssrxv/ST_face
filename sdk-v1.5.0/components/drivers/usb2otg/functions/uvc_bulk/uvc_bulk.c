...
#define UVC_WARN(TAG, ...)    UVC_WARN(TAG, ##__VA_ARGS__)
#else
...
static void uvc_start_stream(struct usb_gadget *gadget)
{
    ...
    if ((m_uvc_stream_state != STREAM_STATE_UNINITED) && (m_uvc_stream_state != STREAM_STATE_IDLE)) {
        UVC_WARN(__func__, "m_uvc_stream_state is %d now! Force it to be STREAM_STATE_UNINITED!", m_uvc_stream_state);
        m_uvc_stream_state = STREAM_STATE_UNINITED;
    }
#if 0
    if (!m_uvc_strm_started) {
...
static int uvc_handle_cam_term_reqs(struct usb_gadget *gadget, const usb_device_request_t *ctrl)
{
    ...
    UVC_WARN(__func__, "Unsupported Interface Ctrl Request, Stall USB ...");
    return -ENOTSUPP;
}
...
static int uvc_handle_ext_unit_reqs(struct usb_gadget *gadget, const usb_device_request_t *ctrl)
{
    ...
    UVC_WARN(__func__, "Unsupported Extension Unit Request, Stall USB ...");
    {
        UVC_DEBUG(__func__, "wIndex 0x%x, wValue 0x%x, wLength 0x%x", 
...
static int uvc_handle_vc_reqs(struct usb_gadget *gadget, const usb_device_request_t *ctrl)
{
    ...
    switch ((wIndex >> 8))
    {
        ...
        default:
            ...
            UVC_WARN(__func__, "Unsupported UVC_VIDEO_CONTROL request:0x%x, stall USB ...\n", wIndex);
...
static int uvc_handle_vs_reqs(struct usb_gadget *gadget, const usb_device_request_t *ctrl)
{
    ...
    switch (wValue)
    {
        case UVC_PROBE_CTRL:
            switch (bRequest)
            {
                case USB_UVC_GET_INFO_REQ:
                    UVC_DEBUG(__func__, "PROBE_CTRL: GET_INFO_REQ");
                    g_ep0_buf[0] = 3;                /* GET/SET requests are supported. */
                    dwc_usb_ep0_send_data(gadget, g_ep0_buf, 1);
                    break;
                case USB_UVC_GET_LEN_REQ:
                    UVC_DEBUG(__func__, "PROBE_CTRL: GET_LEN_REQ");
                ...
                case USB_UVC_GET_DEF_REQ: /* There is only one setting per USB speed. */
                    UVC_DEBUG(__func__, "PROBE_CTRL: GET_CUR/MIN/MAX/DEF_REQ");
                    if (g_set_cur_updated) {
                        ...
                        UVC_DEBUG(__func__, "bFormatIndex is %d", g_commit_ctl.bFormatIndex);
                        UVC_DEBUG(__func__, "bFrameIndex  is %d", g_commit_ctl.bFrameIndex);
                        UVC_DEBUG(__func__, "dwFrameInterval is %d", UGETDW(g_probe_ctl20.dwFrameInterval));
                        g_set_cur_updated = 0;
                    }
                    dwc_usb_ep0_send_data(gadget, &g_probe_ctl20, UVC_MAX_PROBE_SETTING);
                    break;
                case USB_UVC_SET_CUR_REQ:
                    UVC_DEBUG(__func__, "PROBE_CTRL: SET_CUR_REQ");
                    dwc_usb_ep0_recv_data(gadget, &g_commit_ctl, UVC_MAX_PROBE_SETTING);
                    g_set_cur_updated = 1;
                    break;
                default:
                    UVC_WARN(__func__, "Unsupported UVC_PROBE_REQUEST:0x%x, Stall USB ...\n", bRequest);
                    ret = -ENOTSUPP;
                    break;
            }
            break;
        ...
        case UVC_COMMIT_CTRL:
            ...
            {
                case USB_UVC_GET_INFO_REQ:
                    UVC_DEBUG(__func__, "COMMIT_CTRL: GET_INFO_REQ");
                    g_ep0_buf[0] = 3;                        /* GET/SET requests are supported. */
                    dwc_usb_ep0_send_data(gadget, g_ep0_buf, 1);
                    break;
                case USB_UVC_GET_LEN_REQ:
                    UVC_DEBUG(__func__, "COMMIT_CTRL: GET_LEN_REQ");
                    g_ep0_buf[0] = UVC_MAX_PROBE_SETTING;
                    dwc_usb_ep0_send_data(gadget, g_ep0_buf, 1);
                    break;
                case USB_UVC_GET_CUR_REQ:
                    UVC_DEBUG(__func__, "COMMIT_CTRL: GET_CUR_REQ");
                    dwc_usb_ep0_send_data(gadget, &g_probe_ctl20, UVC_MAX_PROBE_SETTING);
                    break;
                case USB_UVC_SET_CUR_REQ:
                    UVC_DEBUG(__func__, "COMMIT_CTRL: SET_CUR_REQ");
                    ...
                    UVC_DEBUG(__func__, "bFormatIndex is %d", g_commit_ctl.bFormatIndex);
                    UVC_DEBUG(__func__, "bFrameIndex  is %d", g_commit_ctl.bFrameIndex);
                    UVC_DEBUG(__func__, "dwMaxVideoFrameSize(commit): %d", UGETDW(g_commit_ctl.dwMaxVideoFrameSize));
                    UVC_DEBUG(__func__, "dwMaxVideoFrameSize(probe):  %d", UGETDW(g_probe_ctl20.dwMaxVideoFrameSize));
                    UVC_DEBUG(__func__, "dwMaxPayloadTransferSize(commit): %d", UGETDW(g_commit_ctl.dwMaxPayloadTransferSize));
                    UVC_DEBUG(__func__, "dwMaxPayloadTransferSize(probe):  %d", UGETDW(g_probe_ctl20.dwMaxPayloadTransferSize));
                    dwc_usb_ep0_recv_data(gadget, &g_commit_ctl, UVC_MAX_PROBE_SETTING);
                    uvc_start_stream(gadget);
                    break;

                default:
                    UVC_WARN(__func__, "Unsupported UVC_COMMIT_REQUEST:0x%x, Stall USB ...\n", bRequest);
                    ret = -ENOTSUPP;
                    break;
            }
            break;
        ...
        default:
            UVC_WARN(__func__, "Unsupported VIDEO STREAM REQUEST:0x%x, Stall USB ...\n", wValue);
            ret = -ENOTSUPP;
            break;
    }
    return ret;
...
static int video_in_ep_complete(usb_ep_t *ep, usb_request_t *req)
{
    ...
    switch (state) {
        ...
        case STREAM_STATE_UNINITED:
            UVC_WARN(__func__, "video_in_ep: STREAM_STATE_UNINITED");
            break;
        case STREAM_STATE_INITED:
            UVC_WARN(__func__, "video_in_ep: STREAM_STATE_INITED");
            break;
        case STREAM_STATE_IDLE:
            UVC_WARN(__func__, "video_in_ep: STREAM_STATE_IDLE");
            break;
        default:
    ...
    switch (state) {
        case STREAM_STATE_BUSY:
            ...
            if (req->actual) {
                ...
                UVC_WARN(__func__, "req->actual is 0! switch to STREAM_STATE_IDLE");
                state = STREAM_STATE_IDLE;
            }
            usb_func_set_stream_state(req, state);
            break;
        case STREAM_STATE_LAST_DATA:
            ...
            break;
        case STREAM_STATE_UNINITED:
            UVC_WARN(__func__, "ext_cmd_out_ep: STREAM_STATE_UNINITED");
            break;
        case STREAM_STATE_INITED:
            UVC_WARN(__func__, "ext_cmd_out_ep: STREAM_STATE_INITED");
            break;
        case STREAM_STATE_IDLE:
            UVC_WARN(__func__, "ext_cmd_out_ep: STREAM_STATE_IDLE");
            break;
        default:
...
static void ext_resp_in_ep_complete(usb_ep_t *ep, usb_request_t *req)
{
    ...
    switch (state) {
        ...
        case STREAM_STATE_LAST_DATA:
            ...
            break;
        case STREAM_STATE_UNINITED:
            UVC_WARN(__func__, "ext_resp_in_ep: STREAM_STATE_UNINITED!");
            break;
        case STREAM_STATE_INITED:
            UVC_WARN(__func__, "ext_resp_in_ep: STREAM_STATE_INITED");
        case STREAM_STATE_IDLE:
            UVC_WARN(__func__, "ext_resp_in_ep: STREAM_STATE_IDLE");
            break;
        default:
...
static void complete(usb_ep_t *ep, usb_request_t *req)
{
	...
	switch (req->status) {
		...
		default:
            ...
            UVC_WARN(__func__, "ep 0x%x, req 0x%x, status %d", ep, req, req->status);
			req->length = DWC_REQ_SIZE;
            ret = ep->ops->queue(ep, req, GFP_ATOMIC);
			if (ret == 0) {
				return;
...
static void ep0_complete(usb_ep_t *ep, usb_request_t *req)
{
    ...
    if (req->status || req->actual != req->length) {
        UVC_WARN(__func__, "setup complete --> %d, %d/%d", req->status, req->actual, req->length);
    }
...
static int bind(struct usb_gadget *gadget, struct usb_gadget_driver *driver)
{
    ...
    if (m_usb_function_binded) {
        UVC_WARN(__func__, "Gadget has been bound, unbind it firstly!", gadget);
        unbind(gadget);
    }
    ...
    if (UT_GET_TYPE(ctrl->bmRequestType) == UT_STANDARD) {
        switch (ctrl->bRequest) {
            ...
            case UR_SYNCH_FRAME:
                ...
                break;
            default:
                UVC_WARN(__func__, "(%d) Unsupported bmRequestType 0x%x, bRequest 0x%x", 
        ...
        if ((ctrl->bmRequestType == USB_UVC_GET_REQ_TYPE) || 
            ...
            if (ctrl->bRequest == USB_SET_INTF_REQ_TYPE) {
                if ((windex == UVC_STREAM_INTERFACE) && (wvalue == 0)) {
                    ...
                    UVC_WARN(__func__, "unkown index(%d), do nothing...", windex);
                    dwc_usb_ep0_wait_nrdy(gadget);
                    ret = 0;
                }
            }
            else {
                UVC_WARN(__func__, "unkown bRequest(%d), do nothing...", ctrl->bRequest);
                dwc_usb_ep0_wait_nrdy(gadget);
                ret = 0;
            }
        }
        ...
        UVC_WARN(__func__, "(%d) UT_VENDOR: bmRequestType 0x%x, bRequest 0x%x", 
                    __LINE__, ctrl->bmRequestType, ctrl->bRequest);
        ret = -ENOTSUPP;
    }
    else {
...
static int usb_bulk_commit_frame(frame_buf_t *cur)
{
    ...
    if (ep == NULL) {
        /*UVC_WARN(__func__, "video_in_ep is NULL!");*/
        frame_mgr_decr_ref(cur);
        ret = -1;
        goto out;
    }
    ...
    if (req == NULL) {
        /*UVC_WARN(__func__, "video_in_req is NULL!");*/
        frame_mgr_decr_ref(cur);
        ret = -1;
        goto out;
    }
    ...
    switch (state) {
        ...
        case STREAM_STATE_LAST_DATA:
            if (pre_remain_size != m_remain_size) {
                ...
                /*UVC_WARN(__func__, "The pre_remain_size is equal to m_remain_size %d", m_remain_size);*/
                timeout--;
            }
            pre_remain_size = m_remain_size;
            if (timeout <= 0) {
                // Check timeout, though it is unlikely.
                UVC_WARN(__func__, "Timeout! m_remain_size is always %d!", m_remain_size);
...
static int usb_bulk_acm_recv_buffer(void *buffer, int *bytes, int wait_option)
{
    ...
    while (remain > 0) {
        ...
        configASSERT(acm_buf->buffer != NULL);
        /*UVC_WARN(__func__, " %d, %d, %d", acm_buf->offset, acm_buf->count, acm_buf->size);*/
...
static int  uvc_bulk_init(uvc_param_t *param)
{
    ...
    UVC_DEBUG(__func__, "set max_payload_size as %d", param->max_payload_size);
...
UDEVICE_EXPORT(UCLASS_USB_GARGET,   uvc_bulk,    0,      &m_uvc_bulk_gadget,    NULL);
