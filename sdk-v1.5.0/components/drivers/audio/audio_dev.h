...
typedef struct audio_dev_config {
    ...
    int initialize_i2s;                    /** initialize i2s or not*/
} audio_dev_config_t;
...
typedef struct audio_frame_buf audio_frame_buf_t;
...
typedef struct audio_dev_ops {
    ...
    int (*mic_get_frame)(audio_dev_t *dev, audio_frame_buf_t* frame, uint32_t timeout_ms);        /** get audio frame from mic*/
    int (*speaker_play_frame)(audio_dev_t *dev, audio_frame_buf_t *frame, uint32_t timeout_ms);   /** send audio frame to speaker*/
} audio_dev_ops_t;
...
/**
 * @brief Get audio frame from mic
 *
 * @note since thers is only one i2s in the chip, so this func will directly call
 *       i2s api to read audio data from mic, so it's no matter whether dev provide
 *       this function or not
 *
 * @param[in] dev device instance to operate on
 * @param[in,out] frame audio frame get from mic
 * @param[in] timeout_ms timeout in miniseconds
 * @return int, 0 for success and other for fail
 */
 ...
 *
 * @note since thers is only one i2s in the chip, so this func will directly call
 *       i2s api to write audio data, so it's no matter whether dev provide this
 *       function or not
 *
 * @param[in] dev device instance to operate on
 * @param[in] frame audio frame to play
 * @param[in] timeout_ms timeout in miniseconds
 * @return int, 0 for success and other for fail
 */
 ...
 *
 * @param[in] samples samples to convert to sample rate
