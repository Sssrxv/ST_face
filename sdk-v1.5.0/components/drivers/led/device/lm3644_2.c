...
int lm3644_init2(led_dev_t *dev)
{
    ...
    int timeout_ms = dev->timeout_ms[CHANNEL_0];
...
int lm3644_enable2(led_dev_t *dev, led_channel_t chn)
{
    lm3644_led_t* _dev = (lm3644_led_t*)dev->priv;
    i2c_device_number_t i2c_num = _dev->i2c_num;
    ...
    if (chn == CHANNEL_ALL) {
        dev->is_enable[CHANNEL_0] = 1;
        dev->is_enable[CHANNEL_1] = 1;
    } else {
        dev->is_enable[chn] = 1;
    }
...
int lm3644_disable2(led_dev_t *dev, led_channel_t chn)
{
    lm3644_led_t* _dev = (lm3644_led_t*)dev->priv;
    i2c_device_number_t i2c_num = _dev->i2c_num;
    uint8_t i2c_addr = _dev->i2c_addr;
    ...
    if (chn == CHANNEL_ALL) {
        dev->is_enable[CHANNEL_0] = 0;
        dev->is_enable[CHANNEL_1] = 0;
    } else {
        dev->is_enable[chn] = 0;
    }
...
int lm3644_suspend2(led_dev_t *dev, led_channel_t chn)
{
#if (defined USE_SENSOR_OV02B_STROBE)
    int ret = lm3644_disable2(dev, chn);
    if (ret == 0) {
        if (chn == CHANNEL_ALL) {
            dev->is_suspend[CHANNEL_0] = 1;
            dev->is_suspend[CHANNEL_1] = 1;
        } else {
...
int lm3644_resume2(led_dev_t *dev, led_channel_t chn)
{
#if (defined USE_SENSOR_OV02B_STROBE)
    int ret = lm3644_enable2(dev, chn);
    if (ret == 0) {
        if (chn == CHANNEL_ALL) {
            dev->is_suspend[CHANNEL_0] = 0;
            dev->is_suspend[CHANNEL_1] = 0;
        } else {
...
int lm3644_get_current_range2(led_dev_t *dev, led_channel_t chn, float *min, float *max, float *step)
{
    lm3644_get_lock(dev);
    *min = CURRENT_MIN_MA;
    *max = CURRENT_MAX_MA;
...
int lm3644_set_current_level2(led_dev_t *dev, led_channel_t chn, led_current_level_t level)
{
    lm3644_get_lock(dev);
...
int lm3644_set_current2(led_dev_t *dev, led_channel_t chn, float current_ma)
{
    int ret = -1;
    ...
    ret = lm3644_set_current_level2(dev, chn, level);

    return ret;
}

int lm3644_set_timeout_ms2(led_dev_t *dev, led_channel_t chn, int timeout_ms)
{
    dev->timeout_ms[CHANNEL_0] = timeout_ms;
    enum lm3644_time_out_val timeout_level = timeout_ms2level(timeout_ms);

    return lm3644_set_flash_timeout(dev, timeout_level);
}

int lm3644_trigger2(led_dev_t *dev, led_channel_t chn)
{
    ...
    if (!dev->is_suspend[chn_idx])
    {
        if (dev->is_enable[chn_idx])
        {
...
static led_dev_t lm3644 = {
    ...
    .is_initialized = 0,
    .is_enable[CHANNEL_0] = 0,
    .is_enable[CHANNEL_1] = 0,
    .is_suspend[CHANNEL_0] = 0,
    .is_suspend[CHANNEL_1] = 0,
    // .timeout_ms = 30,
    .timeout_ms[CHANNEL_0] = 40, // this vlaue should be the same with {TIMING_REG, 0x13}
    .timeout_ms[CHANNEL_1] = 40, // this vlaue should be the same with {TIMING_REG, 0x13}
    .priv = &led0,
};
