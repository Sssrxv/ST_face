...
int gpio_led_enable(led_dev_t *dev, led_channel_t chn)
{
...
int gpio_led_disable(led_dev_t *dev, led_channel_t chn)
{
...
int gpio_led_suspend(led_dev_t *dev, led_channel_t chn)
{
...
int gpio_led_resume(led_dev_t *dev, led_channel_t chn)
{
...
int gpio_led_set_timeout(led_dev_t *dev, led_channel_t chn, int timeout_ms)
{
...
int gpio_led_trigger(led_dev_t *dev, led_channel_t chn)
{
    configASSERT(dev->priv != NULL);
    gpio_led_t* _dev = (gpio_led_t*)dev->priv;
    uint8_t pin = _dev->pin;
    int timeout_ms = dev->timeout_ms[CHANNEL_0];
    if (!dev->is_suspend[CHANNEL_0])
    {
        if (dev->is_enable[CHANNEL_0])
        {
...
static led_dev_t gpio_led = {
    ...
    .timeout_ms[CHANNEL_0] = 30,
    .timeout_ms[CHANNEL_1] = 30,
    .priv = &gpio3,
};
